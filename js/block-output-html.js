...
function bbCodeJSReturn(block, update=false) {
  let jsOutputText = '';
  try {
    jsOutputText = block.getContents();
  } catch(err) {
    console.log(`Error: ${err.message}`);
    jsOutputText = `Error: See Console`;
  }
  if (update) return jsOutputText;
  return `<span ${block.getPropertiesOutput()}>${jsOutputText}</span>`;
}

// evaluates js code and returns the result
function bbCodeJSEval(block, update=false) {
  let jsOutputText = '';
  try {
...
function bbCodeReturnContents(block) {
  let open = `<span ${block.getPropertiesOutput()}>`;
