...
/* global parseBBFormatting, parseRawFormatting, removeContentLineBreaks, Slide */
...
function addSlideToDict(array) {
    ...
    for (let i = firstLine; i < array.length; i++) {
        if (array[i].toLowerCase().substring(0,9) === '=project$') {
            ...
            let textRaw =  slide[0].split('===\n')[1].trim();
            let buttonRaw = slide.slice(1);
...
function parseRaw(text) {
    ...
    for (let i = 0; i < rawCards.length; i++) {
        ...
        for (let j = 0; j < temp.length; j++) {
            temp[j] = parseRawFormatting(temp[j]);
            temp[j] = parseBBFormatting(temp[j]);
        }
